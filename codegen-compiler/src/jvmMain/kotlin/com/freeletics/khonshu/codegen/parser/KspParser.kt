...
internal fun KSFunctionDeclaration.toHostActivityData(
    ...
    if (annotation.scope == activityScope) {
        logger.error(
            "Using ActivityScope as scope for @NavHostActivity is not allowed. Each " +
                "@NavHostActivity should define its own unique scope. ActivityScope will be automatically " +
                "included in each graph generated for @NavHostActivity and can be used to contribute " +
                "to all activities.",
            annotation,
        )
        return null
